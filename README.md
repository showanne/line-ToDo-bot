...
- **情境式智慧導覽 (Smart Toolbar)**：所有卡片底部現在配備了智慧導覽列，會根據當前查看的內容動態變換按鈕：
  - **主入口**：顯示 `[🏷️標籤] [📍地點] [➕新增]`。
  - **特定分類中**：自動出現 `[➕新增至 此分類]`，點擊即代入路徑並開啟鍵盤。
  - **搜尋結果中**：提供快速切換至其他標籤或地點的捷徑。
...
- **功能**: 將資料庫內所有資料表（分類、標籤、事項及其關聯）轉換為 SQL `INSERT` 語句。
- **用途**: 用於手動備份或將資料遷移至另一個資料庫執行。

### GET `/health`

- **用途**: 供 UptimeRobot 等工具監測服務狀態。
